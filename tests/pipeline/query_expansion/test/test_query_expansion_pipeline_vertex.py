...
def test_query_expansion_to_retrieval_payload_with_vertex() -> None:
    ...
    assert payload["metadata"]["clause_text"] == clause

    assert payload["law"]["bm25_keywords"]
    assert payload["law"]["dense_query"]

    assert payload["case"]["bm25_keywords"]
    assert payload["case"]["issue_query"]
    assert payload["case"]["fact_pattern_query"]

    assert payload["counsel"]["bm25_keywords"]
    assert payload["counsel"]["question_query"]
    assert payload["counsel"]["answer_query"]
